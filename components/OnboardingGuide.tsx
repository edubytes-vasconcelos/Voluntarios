...
const OnboardingGuide: React.FC<OnboardingGuideProps> = ({
  ...
  const [currentStep, setCurrentStep] = useLocalStorage<number>(`onboarding_step_${organizationId}`, 0);
  ...
  const handleSkipOnboarding = () => {
      if (window.confirm("Tem certeza que deseja pular o onboarding? Você pode configurar tudo manualmente depois.")) {
      ...
      <div className="mt-6 flex justify-end">
       ...
       {volunteers.length > 0 && ministries.length > 0 && eventTypes.length > 0 && services.length > 0 && currentStep === 3 && ( // Simple check that something was added
           <div className="mt-4 p-2 bg-brand-bg rounded-lg text-sm flex items-center justify-between">
               ...
               <button onClick={handleNextStep} className="text-brand-primary hover:underline flex items-center gap-1">Finalizar <ChevronRight size={14}/></button>
           </div>
       )}
    </div>,
