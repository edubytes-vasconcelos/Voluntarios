...
import { Loader2, LogIn, Mail, Lock, UserPlus, CheckCircle, Building2, User, AlertCircle, Database, RefreshCw, Copy, Sparkles, Eye, EyeOff, KeyRound, Info } from 'lucide-react';
  ...
  const handleRegisterChurch = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (authError && authError.message.includes('already registered')) {
            ...
            authData.session = loginData.data.session; // Fix data.session reference
        } else if (authError) {
            throw authError;
        }
